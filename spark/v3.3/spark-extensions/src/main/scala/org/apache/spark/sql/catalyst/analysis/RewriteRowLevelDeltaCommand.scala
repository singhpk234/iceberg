...
trait RewriteRowLevelDeltaCommand extends Rule[LogicalPlan] {
  ...
  private def newLazyProjection(
      plan: LogicalPlan, attrs: Seq[Attribute], usePlanTypes: Boolean): ProjectingInternalRow = {
  ...
  protected def resolveRowIdAttrs(
      relation: DataSourceV2Relation, operation: RowLevelOperation): Seq[AttributeReference] = {
