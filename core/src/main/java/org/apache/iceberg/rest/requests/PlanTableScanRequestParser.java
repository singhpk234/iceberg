...
public class PlanTableScanRequestParser {
  ...
  public static void toJson(PlanTableScanRequest request, JsonGenerator gen) throws IOException {
    ...
    JsonNode filterJson = request.filterJson();
    if (filterJson != null) {
      gen.writeFieldName(FILTER);
      gen.writeTree(filterJson);
    }
  ...
  public static PlanTableScanRequest fromJson(JsonNode json) {
    ...
    if (json.has(FILTER)) {
      ...
      // Validate that it's not explicitly null
      Preconditions.checkArgument(
          !filterJson.isNull(), "Cannot parse expression from non-object: null");
    }
